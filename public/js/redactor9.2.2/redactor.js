			...
			if (this.uploadOptions.input)
			{
				...
				$(this.form).ajaxSubmit({
				    success: function(json){
				    	...
				    	if (self.uploadOptions.success)
						{
							self.hideProgressBar();

							if (typeof json.error == 'undefined') self.uploadOptions.success(json);
							else
							{
								self.uploadOptions.error(self, json);
								self.modalClose();
							}
						    self.sync();
						}

						self.el.attr('action', self.element_action);
						self.el.attr('target', '');
				    },
			        error: function(data){alert('Upload failed!'); self.modalClose();}
				});
